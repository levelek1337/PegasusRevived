...
namespace SteveMem
{
    ...
    public partial class Mem
    {
        ...
        public string AddressAddition(string Address, string Offset)
        {
            if (Address == "" || Offset == "") { return ""; }
            if (Offset.StartsWith("-")) { return AddressSubtraction(Address, Offset); }
            return (Convert.ToInt64(Address, 16) + Convert.ToInt64(Offset, 16)).ToString("X2");
        }

        public string AddressSubtraction(string Address, string Offset)
        {
            if (Address == "" || Offset == "") { return ""; }
            return (Convert.ToInt64(Address, 16) - Convert.ToInt64(Offset.Trim('-'), 16)).ToString("X2");
        }
